...
RESOLUTION_COMPARISON_PRINCIPLE = (
    "`verdict` field must be exactly the same. All other fields must be similar"
)
...
class ProvenOracleV2Contract(gl.Contract):
    ...
    def _format_resolution_summary(self, label: str, detail: str) -> str:
        normalized_detail = self._normalize_text(detail, "No additional detail.")
        return f"{label}: {normalized_detail}"

    def _sanitize_webpage_text(self, webpage_text) -> str:
        text = self._normalize_text(webpage_text)
    ...
    def _normalize_resolution_result(self, raw_output) -> dict:
        ...
        verdict = self._normalize_text(
            parsed.get("verdict", parsed.get("outcome", VERDICT_UNDETERMINED)),
            VERDICT_UNDETERMINED,
        ).upper()
        explanation = self._normalize_text(
            parsed.get("explanation", parsed.get("reasoning", "")),
        ...
        if verdict in (VERDICT_UNDETERMINED, VERDICT_UNRESOLVABLE) and not explanation.startswith(
            ...
            explanation = self._format_resolution_summary(verdict, explanation)
...
Confidence: integer 0-100."""
    ...
    def _run_resolution(self, claim: Claim) -> dict:
        ...
        normalize_resolution_result = self._normalize_resolution_result
        build_resolution_prompt = self._build_resolution_prompt
        sanitize_webpage_text = self._sanitize_webpage_text
        format_resolution_summary = self._format_resolution_summary

        def evaluate() -> str:
            try:
                webpage_text = sanitize_webpage_text(
                    gl.get_webpage(resolution_url, mode="text")
                )
            ...
            try:
                ...
                llm_result = gl.exec_prompt(prompt)
                return _json_string(normalize_resolution_result(llm_result))
            except Exception as error:
                return _json_string(
                    {
                        "verdict": VERDICT_UNDETERMINED,
        ...
        result = gl.eq_principle_prompt_comparative(
            evaluate,
            principle=RESOLUTION_COMPARISON_PRINCIPLE,
        )
        return self._normalize_resolution_result(result)
