...
import { moderateClaim, type ClaimModerationInput } from "@/lib/server/claim-moderation";
import {
  getGlobalCooldownMs,
  hashModerationInput,
  moderationInFlight,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const p = moderateClaim({ input, locale })
      .then((result) => {
        // Cache results to avoid burning quota on identical inputs.
        // Allow: longer TTL; Block/Review: shorter TTL to allow policy/prompt iterations.
        const ttlMs =
    ...
    const isRateLimited = /request failed\s*\(429\)|RESOURCE_EXHAUSTED|quota|rate limit/i.test(message);
    if (isRateLimited) {
      // Circuit breaker: stop hammering Gemini for a bit.
